from sqlalchemy import create_engine, Column, Integer, String, DateTime, Date, Time, Boolean, ForeignKey, UniqueConstraint
...
class TheaterMovieDate(Base):
    ...
    __table_args__ = (
        UniqueConstraint('theater_id', 'movie_id', 'show_date'),
    )
...
class MovieFormat(Base):
    ...
    id = Column(Integer, primary_key=True)
    theater_movie_date_id = Column(Integer, ForeignKey('theater_movie_dates.id', ondelete='CASCADE'))
    format_id = Column(Integer, ForeignKey('formats.id', ondelete='CASCADE'))

    theater_movie_date = relationship('TheaterMovieDate', back_populates='movie_formats')
    format = relationship('Format', back_populates='movie_formats')
    showtimes = relationship('Showtime', back_populates='movie_format')

    __table_args__ = (
        UniqueConstraint('theater_movie_date_id', 'format_id'),
    )
...
class Showtime(Base):
    ...
    id = Column(Integer, primary_key=True)
    movie_format_id = Column(Integer, ForeignKey('movie_formats.id', ondelete='CASCADE'))
    ...
    __table_args__ = (
        UniqueConstraint('movie_format_id', 'show_time'),
    )
...
class SeatNotification(Base):
    ...
    __table_args__ = (
        UniqueConstraint('user_email', 'showtime_id', 'seat_number'),
    )